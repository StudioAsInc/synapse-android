...
package com.synapse.social.studioasinc.audiocutter;
...
public class CheapMP3 extends CheapSoundFile {
        ...
        /**
         * MP3 supports seeking into the middle of the file, no header needed, s
o
         * this method is supported to hear exactly what a "cut" of the file sou
nds
        ...
        public void ReadFile(File inputFile) throws java.io.FileNotFoundExceptio
n,
                        java.io.IOException {
                ...
                // No need to handle filesizes larger than can fit in a 32-bit i
nt
                ...
                while (pos < mFileSize - 12) {
                        // Read 12 bytes at a time and look for a sync code (0xF
F)
                        while (offset < 12) {
                                offset += stream.read(buffer, offset, 12 - offse
t);
                        }
                        int bufferOffset = 0;
                        ...
                        if (mProgressListener != null) {
                                boolean keepGoing = mProgressListener.reportProg
ress(pos * 1.0
                                                / mFileSize);
                                if (!keepGoing) {
                                        break;
                        ...
                        if (bufferOffset > 0) {
                                // We didn't find a sync code (0xFF) at position
 0;
                        ...
                        if (mpgVersion == 1) {
                                // MPEG 1 Layer III
                                bitRate = BITRATES_MPEG1_L3[(buffer[2] & 0xF0) >
> 4];
                                sampleRate = SAMPLERATES_MPEG1_L3[(buffer[2] & 0
x0C) >> 2];
                        } else {
                                // MPEG 2 Layer III
                                bitRate = BITRATES_MPEG2_L3[(buffer[2] & 0xF0) >
> 4];
                                sampleRate = SAMPLERATES_MPEG2_L3[(buffer[2] & 0
x0C) >> 2];
                        }
                        ...
                        int frameLen = 144 * bitRate * 1000 / sampleRate + paddi
ng;
...
> 7);
                        ...
                        if (mNumFrames == mMaxFrames) {
                                // We need to grow our arrays. Rather than naive
ly
                                // doubling the array each time, we estimate the
 exact
                                // number of frames we need and add 10% padding.
 In
                                // practice this seems to work quite well, only
one
                                // resize is ever needed, however to avoid patho
logical
                                // cases we make sure to always double the size
at a minimum.

                                mAvgBitRate = mBitrateSum / mNumFrames;
                                int totalFramesGuess = ((mFileSize / mAvgBitRate
) * sampleRate) / 144000;
