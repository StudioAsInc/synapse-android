...
class ProfilePhotoHistoryActivity : AppCompatActivity() {
    ...
    private fun initializeLogic() {
        profilePhotosHistoryList.layoutManager = GridLayoutManager(this, 3)
        ...
        getUserReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (dataSnapshot.exists()) {
                    currentAvatarUri = dataSnapshot.child("avatar").getValue(String::class.java)
                }
            }
            override fun onCancelled(databaseError: DatabaseError) {}
        })

        val getProfileHistoryRef = maindb.getReference("skyline/profile-history").child(auth.currentUser!!.uid)
    ...
    private fun uploadProfilePhoto(filePath: String) {
        loadingDialog(true)
    ...
    private fun deleteProfileImage(key: String, type: String, uri: String, publicId: String?) {
        if (uri == currentAvatarUri) {
            mSendMap["avatar"] = "null"
            mSendMap["avatar_history_type"] = "local"
        ...
        if (type == "cloudinary" && publicId != null) {
            FasterCloudinaryUploader.delete(publicId, object: FasterCloudinaryUploader.DeleteCallback {
                override fun onSuccess() {
                    maindb.getReference("skyline/profile-history/${auth.currentUser!!.uid}/$key").removeValue()
                    getReference()
                }
                override fun onError(error: String) {
    ...
    inner class ProfilePhotosHistoryListAdapter(private val data: ArrayList<HashMap<String, Any>>) :
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            val v = LayoutInflater.from(parent.context).inflate(R.layout.dp_history_cv, parent, false)
            return ViewHolder(v)
        }
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            Glide.with(this@ProfilePhotoHistoryActivity).load(Uri.parse(item["image_url"].toString())).into(profile)
            ...
            holder.itemView.setOnClickListener {
                ...
                mSendMap["avatar"] = if (newUri == currentAvatarUri) "null" else newUri
                mSendMap["avatar_history_type"] = if (newUri == currentAvatarUri) "local" else item["type"].toString()
                maindb.getReference("skyline/users/${auth.currentUser!!.uid}").updateChildren(mSendMap)
